...
class ImageBenchmarker:
    def __init__(self, img_benchmark_type="simGetImages", viz_image_cv2=False, save_images=False, img_type="scene"):
    ...
    def image_callback_benchmark_simGetImages(self):
        ...
        if DEBUG:
            if response.pixels_as_float:
                print(f"Type {response.image_type}, size {len(response.image_data_float)}," f"height {response.height}, width {response.width}")
            else:
                print(f"Type {response.image_type}, size {len(response.image_data_uint8)}," f"height {response.height}, width {response.width}")
...
def main(args):
    image_benchmarker = ImageBenchmarker(img_benchmark_type=args.img_benchmark_type, viz_image_cv2=args.viz_image_cv2, save_images=args.save_images, img_type=args.img_type)
